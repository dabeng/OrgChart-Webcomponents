...
var OrgChart = function (_HTMLElement) {
  ...
  function OrgChart(options) {
    ...
    _this.dataset.options = JSON.stringify(opts);
    _this.setAttribute('class', 'orgchart' + (opts.chartClass !== '' ? ' ' + opts.chartClass : '') + (opts.direction !== 't2b' ? ' ' + opts.direction : ''));
    if ((typeof data === 'undefined' ? 'undefined' : _typeof(data)) === 'object') {
      // local json datasource
      _this.buildHierarchy(_this, opts.ajaxURL ? data : _this._attachRel(data, '00'), 0);
    } else if (typeof data === 'string' && data.startsWith('#')) {
      // ul datasource
      _this.buildHierarchy(_this, _this._buildJsonDS(document.querySelector(data).children[0]), 0);
    } else {
      // ajax datasource
      var spinner = document.createElement('i');

      spinner.setAttribute('class', 'fa fa-circle-o-notch fa-spin spinner');
      _this.appendChild(spinner);
      _this._getJSON(data).then(function (resp) {
        ...
        var spinner = that.querySelector('.spinner');

        spinner.parentNode.removeChild(spinner);
      });
    }
    _this.addEventListener('click', _this._clickChart.bind(_this));

    if (opts.pan && opts.chartContainer) {
      var chartContainer = document.querySelector(opts.chartContainer);

      chartContainer.style.overflow = 'hidden';
      _this.addEventListener('mousedown', _this._onPanStart.bind(_this));
      _this.addEventListener('touchstart', _this._onPanStart.bind(_this));
      document.body.addEventListener('mouseup', _this._onPanEnd.bind(_this));
      document.body.addEventListener('touchend', _this._onPanEnd.bind(_this));
    }

    if (opts.zoom && opts.chartContainer) {
      var _chartContainer = document.querySelector(opts.chartContainer);

      _chartContainer.addEventListener('wheel', _this._onWheeling.bind(_this));
      _chartContainer.addEventListener('touchstart', _this._onTouchStart.bind(_this));
      document.body.addEventListener('touchmove', _this._onTouchMove.bind(_this));
      document.body.addEventListener('touchend', _this._onTouchEnd.bind(_this));
    }
    return _this;
  }
  ...
  _createClass(OrgChart, [{
    ...
    value: function showChildren(node) {
      ...
      this._one(descendants[0], 'transitionend', function (event) {
        _this8._removeClass(descendants, 'slide');
        if (_this8._isInAction(node)) {
          _this8._switchVerticalArrow(node.querySelector('.bottomEdge'));
        }
      }, this);
    ...
    value: function _buildSiblingNode(nodeChart, nodeData, callback) {
      ...
      if (nodeChart.parentNode.nodeName === 'TD') {
        (function () {
          var temp = _this10._prevAll(nodeChart.parentNode.parentNode);
        ...
        (function () {
          ...
          that.buildHierarchy.call(that, that, nodeData, 0, function () {
            if (++nodeCount === siblingCount) {
              ...
              var temp3 = _this10._siblings(nodeChart.parentNode);
    ...
    value: function _clickHorizontalEdge(event) {
      ...
      if (siblingsState.exist) {
        ...
        if (siblings.some(function (el) {
          ...
          return _this11._isVisible(node) && node.classList.contains('slide');
        })) {
          return;
        }
    ...
    value: function _onDragStart(event) {
      ...
      if (this.style.transform) {
        ...
        var transValues = this.style.transform.split(','),
    ...
    value: function _createNode(nodeData, level) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var inEdit = that.dataset.inEdit,
            isHidden = void 0;
    ...
    value: function _loopChart(chart) {
      ...
      if (chart.children[3]) {
        Array.from(chart.children[3].children).forEach(function (el) {
          ...
          subObj.children.push(_this12._loopChart(el.firstChild));
        });
      }
      return subObj;
    }
    ...
    value: function _onTouchEnd(event) {
      if (this.dataset.pinching) {
        ...
        var diff = this.dataset.pinchDistEnd - this.dataset.pinchDistStart;
