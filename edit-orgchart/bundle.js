...
/***/ function(module, exports, __webpack_require__) {
	...
	function toggleNodeType() {
	  if (this.value === 'parent') {
	    ...
	    Array.from(document.getElementById('new-nodelist').children).slice(1).forEach(function (newNode) {
	...
	function removeInputs() {
	  var inputs = Array.from(document.getElementById('new-nodelist').children);
	...
	function addNodes(orgchart) {
	  ...
	  if (nodeType.value === 'parent') {
	    if (!document.querySelector('#chart-container').children.length) {
	...
	document.addEventListener('DOMContentLoaded', function () {
	  var orgchart = void 0,
	      datascource = {
	    'name': 'Ball game',
	    'children': [{ 'name': 'Football' }, { 'name': 'Basketball' }, { 'name': 'Volleyball' }]
	  };
	  ...
	  document.getElementById('btn-add-nodes').addEventListener('click', function () {
	    return addNodes(orgchart);
	  });
	  document.getElementById('btn-delete-nodes').addEventListener('click', function () {
	    return deleteNodes(orgchart);
	  });
